...
CHECKPOINT_PATH = REPOSITORY / "models" / "Goal_Refinement_Learning_latest.pt"
...
GAMMA, GAE_LAMBDA, VALUE_COEFFICIENT = 0.98, 0.99, 1.0
...
# --recalibrate converts these relative weights into coefficients for the current setup.
# position, velocity, goal difference, smoothness, saturation, torque, penetration, root Z
REWARD_WEIGHTS = np.array((.30, .25, 0., 0., .15, .10, 0., .20), np.float32)
...
def rollout_chunk(specs, sampler, runtime, actor, critic, normalizer, action_std,
    ...
    for t in range(maximum):
        ...
        # penetration is diagnostic only; self-collision learning is supervised. Root-Z
        # tracking is separated from global body-position tracking and calibrated itself.
        ...
        components = torch.stack((position_distance.square(), velocity_distance.square(),
            goal_difference, smooth, saturation, utilization, depth, height_drop.square()), -1)
    ...
    goal_positions = torch.stack(goal_position_list, 1).cpu()
    goal_velocities = torch.stack(goal_velocity_list, 1).cpu()
    rewards = -(components * torch.as_tensor(REWARD_COEFFICIENTS)).sum(-1)
...
def load_reward_calibration(action_std, kp, kd):
    ...
    with np.load(REWARD_CALIBRATION_PATH) as saved:
        ...
        compatible = (schema == 4 and np.array_equal(saved["settings"], settings)
    ...
    if coefficients.shape != (8,) or difference.shape != (2,) or not np.isfinite(coefficients).all() \
...
def recalibrate_rewards(sampler, runtime, action_std, joint_low, joint_high,
    ...
    scaled = np.divide(REWARD_WEIGHTS, stds, out=np.zeros(8), where=(REWARD_WEIGHTS > 0) & (stds > 0))
    ...
    settings = np.array((DURATION_STD, DURATION_MIN, DURATION_MAX, ACTION_HZ,
                         EXPLORING_REGION, STD_MULTIPLIER), np.float64)
    np.savez(REWARD_CALIBRATION_PATH, schema=4, settings=settings, smooth=SMOOTH,
    ...
    labels = ("Tracking position squared", "Tracking velocity squared", "Altered-goal difference",
              ...
              "Directional torque utilization", "Penetration depth", "Root-Z tracking squared")
    lines = ["Current reward calibration", "==========================", "",
        ...
        f"Fall is diagnostic only; calibration fall rate={falls.mean():.6g}.", "",
...
def summarize(records):
    ...
    csum, count = stats[:8], stats[8]; rmean, emean = stats[25]/stats[29], stats[27]/stats[29]
    variance = stats[26]/stats[29]-rmean*rmean; error_variance = stats[28]/stats[29]-emean*emean
    result = {"cost":float(csum @ REWARD_COEFFICIENTS/count),
        "pos_rmse":float(np.sqrt(csum[0]/count)), "pos_p95":float(p95[0]),
        "vel_rmse":float(np.sqrt(csum[1]/count)), "vel_p95":float(p95[1]),
        "root_z_rmse":float(np.sqrt(csum[7]/count)),
        "goal_diff_rmse":float(np.sqrt(stats[15]/stats[16])),
        "goal_velocity_diff_rmse":float(np.sqrt(stats[17]/stats[18])),
        "goal_combined":float(stats[9]/stats[10]), "smooth":float(stats[13]/stats[14]),
        "saturation":float(csum[4]/count), "torque_util":float(csum[5]/count),
        "penetration_depth":float(stats[11]/stats[12]), "joint_clip":float(stats[19]/stats[20]),
        "penetration":float(stats[21]/stats[22]), "altered_track":float(stats[23]/stats[24]),
        "fall_rate":float(stats[30]/stats[31]),
        "explained_variance":float(1-error_variance/variance) if variance > EPS else 0.}
    for index, name in enumerate(("pos", "vel", "goal", "smooth", "sat", "torque", "penetration", "root_z")):
...
def plots(history):
    ...
    for axis, keys, title in zip(axes.flat,
        (("pos_rmse", "validation_pos_rmse", "root_z_rmse", "validation_root_z_rmse"), ("pos_p95", "validation_pos_p95"),
         ("vel_rmse", "validation_vel_rmse"), ("vel_p95", "validation_vel_p95")),
        ("Position / root-Z RMSE (m)", "Position P95 (m)", "Velocity RMSE (m/s)", "Velocity P95 (m/s)")):
        for key in keys:
            values = np.asarray([r.get(key, np.nan) for r in history]); valid = np.isfinite(values)
            if valid.any(): axis.plot(np.asarray(x)[valid], values[valid], label=key)
    ...
    for axis, names, title in zip(axes.flat, keys,
        ...
        for name in names:
            values = np.asarray([r.get(name, np.nan) for r in history]); valid = np.isfinite(values)
            if valid.any(): axis.plot(np.asarray(x)[valid], values[valid], label=name)
    ...
    for axis, key, title in zip(axes.flat, ("cost", "critic_loss", "explained_variance", "critic_grad"),
        ...
        if key == "cost":
            for item in ("pos", "vel", "goal", "smooth", "sat", "torque", "penetration", "root_z"):
                values = np.asarray([r.get(f"weighted_{item}", np.nan) for r in history]); valid = np.isfinite(values)
                if valid.any(): axis.plot(np.asarray(x)[valid], values[valid], label=item)
...
def checkpoint(batch, actor, critic, actor_optimizer, critic_optimizer, history):
    ...
    payload = {"schema":"goal_residual_online_v6", "exploring_region":EXPLORING_REGION,
        "gae_settings":GAE_SETTINGS, "batch":batch, "history":history,
...
# learning rates and GAE settings are authoritative. Optimizer moments are
...
def load_checkpoint(actor, critic, actor_optimizer, critic_optimizer):
    ...
    if saved.get("schema") != "goal_residual_online_v6":
        ...
        while args.stop is None or batch < args.stop:
            ...
            if batch % PRINT_INTERVAL == 0 and primary():
                ...
                print(f"batch={batch} cost={metrics['cost']:.5g} pos={metrics['pos_rmse']:.5g}m "
                    f"root_z={metrics['root_z_rmse']:.5g}m vel={metrics['vel_rmse']:.5g}m/s "
                    f"goal_diff={metrics['goal_diff_rmse']:.5g}m "
